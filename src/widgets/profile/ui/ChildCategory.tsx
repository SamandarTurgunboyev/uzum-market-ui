...
import { z } from 'zod';
import { subCategoryForm } from '../lib/product.validation';

const ChildCategory = () => {
  const queryClient = useQueryClient();

  const form = useForm<z.infer<typeof subCategoryForm>>({
    resolver: zodResolver(subCategoryForm),
    defaultValues: { name_en: '', name_ru: '', name_uz: '', id: '' },
  });

  const { mutate, isPending } = useMutation({
    mutationFn: (body: z.infer<typeof subCategoryForm>) =>
      createChildCategory(body),
    onSuccess: () => {
  ...
  function onSubmit(values: z.infer<typeof subCategoryForm>) {
    mutate(values);
  }
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-full">
        <Label>Eng quyi kategoriya qo`shish</Label>
