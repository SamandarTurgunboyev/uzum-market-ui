            ...
            <div className="flex max-lg:flex-col">
                <div className="w-[70%] max-lg:w-full">
                    <div className="flex gap-5">
                        ...
                        >
                            <CarouselContent className="h-[600px] max-2xl:h-[450px] max-xl:h-[350px]">
                                {Array.from({ length: 6 }).map((_, index) => (
                                    <CarouselItem key={index} className="basis-1/3 max-xl:basis-1/2">
                                        <div className="p-1">
                                            <Image
                        ...
                        >
                            <CarouselContent>
                                {Array.from({ length: 5 }).map((_, index) => (
                                    <CarouselItem key={index} className="basis-1/2 max-md:basis-1/1">
                                        <div className="p-1">
                                            <Image
                                                src="https://placehold.co/300x400.jpg"
                                                width={300}
                    ...
                    >
                        <CarouselContent className="-ml-1">
                            {Array.from({ length: 5 }).map((_, index) => (
                                <CarouselItem key={index} className="basis-1/2 bg-white shadow max-md:basis-1/1 p-4 flex-col items-center ml-1">
                        ...
                        <Button className='w-full mt-4 bg-white shadow text-black font-semibold text-lg' variant={"outline"}>Hammasini ko'rish</Button>
                    </Carousel>
                ...
                <div className="w-[30%] max-lg:mt-5 max-lg:w-full min-lg:p-4">
                    <div className='bg-white shadow p-2'>
                        <p className='font-semibold text-lg inline-block'>{product.price}</p>
                        <span className='ml-4 font-semibold text-sm line-through'>{"49.000 so'm"}</span>
                        <div className='flex mt-5 gap-2 p-2'>
                            <Button className='bg-gray-200 text-black w-[90%]' variant={"ghost"} onClick={() => setAdded(true)}>Mahsulot soni eng kamida 1ta</Button>
                        ...
                        {added &&
                            <div className='p-2 flex justify-between'>
                                <Button className='bg-white shadow w-[10%]' variant={"ghost"}><Minus /></Button>
                                <Button className='bg-white shadow w-[70%]' variant={"ghost"}>1</Button>
                                <Button className='bg-white shadow w-[10%]' variant={"ghost"}><Plus /></Button>
                            </div>
                        }
                        <div className='p-2'>
                            <Button className='w-full' onClick={handleAddToCart}>{"Savatga qo'shish"}</Button>
                        </div>
                        <div className='p-2 flex flex-col gap-4'>
                            <div className='flex items-center gap-4'>
                                <Button className='bg-green-500/50 hover:bg-green-500/50 cursor-pointer' variant={'ghost'}>
                                    <Check />
                                </Button>
                                <p>{"1kishi eng ko'pi bilan 2ta zakaz qila oladi"}</p>
                            </div>
                            <div className='flex items-center gap-4'>
                                <Button className='bg-yellow-500/50 hover:bg-green-500/50 cursor-pointer' variant={'ghost'}>
                                    <Star />
                                </Button>
                                <div className='flex'>
                                    {Array.from({ length: 5 }).map((_, index) => (
                            ...
                            <div>
                                <p className='text-center font-semibold text-md'>To'lov usullari:</p>
                                <div className='flex items-center gap-4'>
